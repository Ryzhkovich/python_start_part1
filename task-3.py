...
class Cell:
    ...
    def __add__(self, other: "Cell") -> "Cell":
        return Cell(self._cell_count + other._cell_count)

    def __sub__(self, other: "Cell") -> "Cell":
        if self._cell_count > other._cell_count:
            return Cell(self._cell_count - other._cell_count)
        print(f"{self._cell_count} - {other._cell_count} < 0, вычетания невозможно")
    ...
    def make_order(self, per_row: int) -> str:
        rows, tail = self._cell_count // per_row, self._cell_count % per_row
        return '\n'.join(['*' * per_row] * rows + (['*' * tail] if tail else []))

    def __str__(self) -> str:
        return f"Клетка из {self._cell_count} ячеек"
...
print(c1)
print(c2)
# Успешные опреации сложения и вычитания
print(c1 + c2)
print(c1 - c2)
# Неудачная операция вычитания
print(c2 - c1)
# Операции умножения и деления
print(c1 * c2)
print(c1 / c2)
# Проверка метода make_order
print((c1 * c2).make_order(29))
